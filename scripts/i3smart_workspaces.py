...
def get_xrandr_screens() -> list:
    ...
    return [x.split()[0] for x in comm.stdout.decode().split("\n")[1:]
...
def move_workspaces_to(i3: i3ipc.connection.Connection,
                       workspaces: list,
                       direction: str) -> None:
    for w in workspaces:
...
def is_workspace_empty(i3: i3ipc.connection.Connection,
                       workspace: i3ipc.replies.WorkspaceReply) -> bool:
    workspace = i3.get_tree().find_by_id(workspace.ipc_data['id'])
    return not workspace or not workspace.workspace().leaves()


def handler_function(i3: i3ipc.connection.Connection,
                     i3thread: threading.Thread,
                     sec: int = 5,
                     direction_HDMI: str = "left",
                     direction_PRIMARY: str = "right") -> None:
    ...
    while True:
        ...
        if HDMI_connected and HDMI_workspaces and not right_move:
            xrandr_auto.main()
            move_workspaces_to(i3,
                               [x for x in HDMI_workspaces
                                if not is_workspace_empty(i3, x)],
                               direction_PRIMARY)
            time.sleep(3)
            i3.command("restart")
            right_move = True
